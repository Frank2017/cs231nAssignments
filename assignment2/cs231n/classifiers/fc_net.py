...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    dA1, dW2, db2 = affine_backward(dl,cache2)
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i in np.arange(self.num_layers ):
        self.params['W'+str(i+1)] = np.random.randn(layer_dims[i], layer_dims[i+1])
    ...
    if self.use_batchnorm:
      # that means restore {'mode':'train'} for (num_layers - 1) times,because
      # between last hidden layers  and output layers dont have to set batch
      # norm layer.
  ...
  def loss(self, X, y=None):
    ...
    out_arr={}
    out_arr[0] = X
    # out_arr store output for each layer-model which include
    # affine-batchnormalization-ReLU-Dropout
    h={}  # store affine's output
    cache1={}  # store affine's cache
    bn={}  # store batch normalization's output
    cache2={}  # store batch normalization's cache
    relu={}  # store relu's output
    cache3={}  # store relu's cache
    cache4={}  # store dropout cache
    for i in np.arange(self.num_layers - 1):
        ...
        if self.use_batchnorm:
            ...
            h[i], cache1[i] = affine_forward(out_arr[i], w, b)
            bn[i],cache2[i] = batchnorm_forward(h[i], gamma, beta, self.bn_params[i])
            out_arr[i+1], cache3[i] = relu_forward(bn[i])
        else:
            out_arr[i+1], cache3[i] = affine_relu_forward(out_arr[i], w, b)
        if self.use_dropout:
            out_arr[i+1], cache4[i] = dropout_forward(out_arr[i+1], self.dropout_param)
    w = self.params['W'+str(self.num_layers)]
    b = self.params['b'+str(self.num_layers)]
    scores, cache = affine_forward(out_arr[self.num_layers-1], w, b)
    pass
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
    ...
    # dh = {}
    t = self.num_layers - 1
    dout[t], grads['W'+str(t+1)], grads['b'+str(t+1)] = affine_backward(dl, cache)
    # print grads['W'+str(t+1)]
    # print cache3
    ...
    for i in np.arange(t):
        if self.use_dropout:
            ...
            if self.use_batchnorm:
                drelu[t-1-i] = relu_backward(ddrop[t-1-i], cache3[t-1-i])
                dbn[t-1-i],grads["gamma"+str(t-i)],grads["beta"+str(t-i)] = batchnorm_backward(drelu[t-1-i],cache2[t-1-i])
                dout[t-1-i],grads['W'+str(t-i)],grads['b'+str(t-i)] = affine_backward(dbn[t-1-i],cache1[t-1-i])
            else:
                dout[t-1-i],grads['W'+str(t-i)],grads['b'+str(t-i)] = affine_relu_backward(ddrop[t-1-i],cache3[t-1-i])
        else:
            if self.use_batchnorm:
                drelu[t-1-i] = relu_backward(dout[t-i], cache3[t-1-i])
                dbn[t-1-i],grads["gamma"+str(t-i)],grads["beta"+str(t-i)] = batchnorm_backward(drelu[t-1-i],cache2[t-1-i])
                dout[t-1-i],grads['W'+str(t-i)],grads['b'+str(t-i)] = affine_backward(dbn[t-1-i],cache1[t-1-i])
            else:
                dout[t-1-i],grads['W'+str(t-i)],grads['b'+str(t-i)] = affine_relu_backward(dout[t-i],cache3[t-1-i])

    # add L2 normalization value to each dW
    for i in np.arange(self.num_layers):
