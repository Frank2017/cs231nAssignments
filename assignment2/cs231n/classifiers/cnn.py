...
class ThreeLayerConvNet(object):
  ...
  def loss(self, X, y=None):
    ...
    reg_loss = 0.5 * self.reg * np.sum(np.sum(W * W) for W in [W1,W2,W3])
    loss = reg_loss + data_loss
    # # backward
    da, dW3, db3 = affine_backward(dl,cache_a)
    dar,dW2, db2 = affine_relu_backward(da,cache_ar)
    dcrp,dW1,db1 = conv_relu_pool_backward(dar, cache_crp)
